...
class TarikTunaiReceiptPage extends StatefulWidget {
  
  final AsyncSnapshot<User?> user;
  final String nominal;
  const TarikTunaiReceiptPage({Key? key, required this.user, required this.nominal}) : super(key: key);

  @override
  State<TarikTunaiReceiptPage> createState() => _TarikTunaiReceiptPageState();
}


class _TarikTunaiReceiptPageState extends State<TarikTunaiReceiptPage> {
  ...
  void decreaseBalance() {
    //decrease firestore balance

    firestore.collection("users").doc(widget.user.data!.email).update({
      "balance": FieldValue.increment(double.parse(widget.nominal.toString().replaceAll('.', '')).round()),
    });
            ...
            child: Container(
              ...
              child: Column(
                ...
                children:  [
                  ...
                  const SizedBox(height: 20,),
                  const Text('Nominal', style: TextStyle(color: Colors.white, fontSize: 14, fontFamily: "OpenSans", fontWeight: FontWeight.w500),),
                  const SizedBox(height: 5,),
          ...
          child: InkWell(
            ...
            child: Center(
              child: Container(
                ...
                child: const Text('Kembali ke Beranda', style: TextStyle(color: Colors.white, fontSize: 18, fontFamily: "OpenSans", fontWeight: FontWeight.w700),),
              ),
            ),
          ),
        )
